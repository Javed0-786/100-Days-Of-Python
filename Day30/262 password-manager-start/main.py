...
def search():
    try:
        ...
        messagebox.showinfo(
            title=f"{website}", message=f"No Data File Found")

    except KeyError:
        messagebox.showinfo(
            title=f"{website}", message=f"{website} not found")
...
label_3.grid(row=3, column=0, pady=10)

entry3 = Entry(width=35)
entry3.grid(row=3, column=1, columnspan=2, pady=10)

search = Button(text="Search", width=15, command=search)
search.grid(row=1, column=3, pady=10)

button_1 = Button(text="Generate Password", command=generatePassword)
button_1.grid(row=3, column=3, pady=10)

button_2 = Button(text="Add", width=36, command=addPassword)
button_2.grid(row=4, column=1, columnspan=2, pady=10)
